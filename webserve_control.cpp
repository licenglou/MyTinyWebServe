...
void WebServeControl::thread_pool_Init()
{
    // 线程池
    m_pool = new threadpool<http_conn>(m_actormodel, m_connPool, m_thread_num);
}

void WebServeControl::trig_mode()
{
    // LT + LT
    if (0 == m_TRIGMode)
    {
...
void WebServeControl::eventListen()
{
    // 网络编程基础步骤
    m_listenfd = socket(PF_INET, SOCK_STREAM, 0);
    assert(m_listenfd >= 0); // 断言函数，0：终止程序
    ...
    assert(ret != -1);
    utils.setnonblocking(m_pipefd[1]);             // 管道写端口，非阻塞
    utils.addfd(m_epollfd, m_pipefd[0], false, 0); // 管道读端口，监听
...
void WebServeControl::eventLoop()
{
    bool timeout = false;
...
bool WebServeControl::dealclinetdata()
{
...
bool WebServeControl::dealwithsignal(bool &timeout, bool &stop_server)
{
    int ret = 0;
    int sig;
    char signals[1024];
...
void WebServeControl::dealwithread(int sockfd)
{
    util_timer *timer = users_timer[sockfd].timer;
...
void WebServeControl::timer(int connfd, struct sockaddr_in client_address)
{
...
// 并对新的定时器在链表上的位置进行调整
void WebServeControl::adjust_timer(util_timer *timer)
{
    time_t cur = time(NULL);
...
void WebServeControl::deal_timer(util_timer *timer, int sockfd)
{
...
void WebServeControl::dealwithwrite(int sockfd)
{
    util_timer *timer = users_timer[sockfd].timer;
    // reactor
    if (1 == m_actormodel)
