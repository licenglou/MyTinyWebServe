...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)
{
    ...
    //将该位置改为\0，用于将前面数据取出(原理：C风格字符串以\0结束)
    *m_url++ = '\0';        
   ...
   //将m_url向后偏移，通过查找，继续跳过空格和\t字符，指向请求资源的第一个字符
    m_url += strspn(m_url, " \t");  //strspn:从m_url开始连续包含" \t"的数目

     //使用与判断请求方式的相同逻辑，判断HTTP版本号（注：请求行构成： 方法  URL  版本号）
...
//这里主要是有些报文的请求资源中会带有http://，这里需要对这种情况进行单独处理
    ...
    {
        m_url += 7;
        m_url = strchr(m_url, '/');   //找出第一次出现/的位置
    }
...
//不符合规则的报文
    ...
    //当url为/时，显示判断界面
    if (strlen(m_url) == 1)
        strcat(m_url, "judge.html");  //复制到m_url后面
...
http_conn::HTTP_CODE http_conn::process_read()
{
    ...
    while ((m_check_state == CHECK_STATE_CONTENT && line_status == LINE_OK) || ((line_status = parse_line()) == LINE_OK))
    {
        ...
        default:
            return INTERNAL_ERROR;      //解析完消息体即完成报文解析，避免再次进入循环，更新line_status
        }
    }
    return NO_REQUEST;
}
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    //printf("m_url:%s\n", m_url);
    const char *p = strrchr(m_url, '/');   //字符最后一次出现的地方
    ...
    else if (*(p + 1) == '1')   //如果请求资源为/1，表示跳转登录页面
    {
        ...
        //将网站根目录和/log.html进行拼接，更新到m_real_file中
        strncpy(m_real_file + len, m_url_real, strlen(m_url_real));
...
bool http_conn::process_write(HTTP_CODE ret)
{
    ...
    case INTERNAL_ERROR:    //内部错误，500
    {
        ...
        add_headers(strlen(error_500_form));   //添加首部字段和空行
